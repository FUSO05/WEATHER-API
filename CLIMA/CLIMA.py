...
# Função para ler pass de um ficheiro
...
# Função para carregar e exibir a imagem apropriada
def exibir_imagem(frame, caminho_imagem):
    try:
        # Função interna para redimensionar a imagem
        def redimensionar_imagem(event=None):
            largura_frame = frame.winfo_width()
            altura_frame = frame.winfo_height()

            img = Image.open(caminho_imagem)
            img = img.resize((largura_frame, altura_frame), Image.Resampling.LANCZOS)
        ...
        label_img.image = img_tk
        label_img.pack(expand=True, fill='both', side='bottom')

        frame.bind('<Configure>', redimensionar_imagem)
...
# Função para exibir especificamente os dados horários
def exibir_graficos_horarios(frame):
    ...
    # Criar subframe para os dados
    frame_dados = ttk.Frame(frame)
    frame_dados.pack(side="top", fill="both", expand=True)

    # Exibir dados em um Treeview
    tree = ttk.Treeview(frame_dados, columns=list(df.columns), show="headings")
    ...
    # Criar subframe para os gráficos
    frame_graficos = ttk.Frame(frame)
    ...
    axs[0, 0].plot(df.index, df['Temperatura (C)'], label='Temperatura (C)', color='red')
    ...
    # Gráfico de Humidade Relativa
    axs[0, 1].plot(df.index, df['Humidade Relativa (%)'], label='Humidade Relativa (%)', color='blue')
    axs[0, 1].set_title('Humidade Relativa (%)')
    axs[0, 1].set_xlabel('Hora')
    axs[0, 1].set_ylabel('Humidade Relativa (%)')
    axs[0, 1].legend()
    axs[0, 1].grid(True)

    # Gráfico de Chuva
    axs[1, 0].plot(df.index, df['Chuva (mm)'], label='Chuva (mm)', color='green')
    axs[1, 0].set_title('Chuva (mm)')
    axs[1, 0].set_xlabel('Hora')
    axs[1, 0].set_ylabel('Chuva (mm)')
    axs[1, 0].legend()
    axs[1, 0].grid(True)

    # Gráfico de Velocidade do Vento
    axs[1, 1].plot(df.index, df['Velocidade do Vento (km/h)'], label='Velocidade do Vento (km/h)', color='orange')
    axs[1, 1].set_title('Velocidade do Vento (km/h)')
    axs[1, 1].set_xlabel('Hora')
    axs[1, 1].set_ylabel('Velocidade do Vento (km/h)')
    axs[1, 1].legend()
    axs[1, 1].grid(True)
    ...
    # Exibir os gráficos no frame
    canvas = FigureCanvasTkAgg(fig, master=frame_graficos)
    canvas.draw()
    canvas.get_tk_widget().pack(side=tk.TOP, fill=tk.BOTH, expand=True)
    
    
# Função para identificar variações de temperatura
def identificar_variacoes_temperatura(dados, popup_window):
    ...
    for indice, derivada in enumerate(derivada_temperatura):
        if derivada > 4:  # Threshold para aumento abrupto
        ...
        elif derivada < -4:  # Threshold para queda abrupta
...
def identificar_mudancas_humidade(dados, popup_window):
    ...
    derivada_humidade = np.gradient(humidade, tempo)
    ...
    max_diff_date_aumento = None
    max_diff_date_queda = None
    for indice, derivada in enumerate(derivada_humidade):
        if derivada > 20:  # Threshold para aumento abrupto
            max_diff_aumento = derivada
            max_diff_date_aumento = dados.iloc[indice]['Data']
            message = f"Aumento abrupto na derivada da humidade em {dados.iloc[indice]['Data']} | {dados.iloc[indice]['Hora']}: {max_diff_aumento}"
            add_message(popup_window, message)
        elif derivada < -20:  # Threshold para queda abrupta
            max_diff_queda = derivada
            max_diff_date_queda = dados.iloc[indice]['Data']
...
def identificar_condicoes_furacoes_tornados(dados, popup_window):
    ...
    # Incializar uma flag para dar track quando email é enviado (garantir que apenas uma email é enviado)
    email_sent = False
    ...
    for _, linha in dados.iterrows():
        ...
        if vento_atual > 100:  # Threshold para identificar tornados/furacões
            ...
            if not email_sent:
                ...
                email_sent = True  # Flag para True quando email é enviado

    return furacoes_tornados


# Função para identificar condições proícias para inundações
def identificar_condicoes_inundacoes(dados, popup_window):
    ...
    for _, linha in dados.iterrows():
        ...
        if chuva_atual > 45:  # Threshold para identificar inundações
            inundacoes.append((linha['Data'], linha['Hora'], chuva_atual))
            message = f"Condicoes propicias para inundacoes encontradas em {linha['Data']} {linha['Hora']}: Quantidade de Chuva {chuva_atual} mm"
            add_message(popup_window, message)
            if not email_sent:
...
# Função para buscar os dados meteorológicos históricos
...
def buscar_dados_clima(city_name, start_hour, end_hour):
    ...
    if latitude_user is None or longitude_user is None:
        return None  # Sair da função se as coordenadas não forem encontradas
    ...
    params = {
        ...
        "current": ["temperature_2m", "relative_humidity_2m", "is_day", "rain", "cloud_cover", "wind_speed_10m"],
        "hourly": ["temperature_2m", "relative_humidity_2m", "rain", "wind_speed_10m"],
...
def get_30days(response, output_file):
   ...
   # Obter as coordenadas da cidade fornecida pelo user
    geolocator = Nominatim(user_agent="my_geocoder")
    ...
    # Configurar a sessão para cache e tentativas de retransmissão
    cache_session = requests_cache.CachedSession('.cache', expire_after=-1)
    retry_session = retry(cache_session, retries=5, backoff_factor=0.2)
    openmeteo = openmeteo_requests.Client(session=retry_session)

    # Definir as datas para 30 dias atrás e hoje
    data_atual = datetime.date.today()
    data_30_dias_atras = data_atual - datetime.timedelta(days=30)

    dados_30_dias = []

    # Fazer a chamada à API para os dados do dia
    url = "https://api.open-meteo.com/v1/forecast"
    for i in range(30):
        ...
        # Extrair os valores relevantes dos dados de resposta
        ...
        # Iterar sobre os valores de cada hora do dia
        for j in range(len(Temperatura)):
            # Organizar os dados da hora no formato desejado
            dado_hora = {
...
# Função para recolher dados de 15 dias futuros
def get_15days_futuro(response, output_file):
    ...
    # Definir a data atual e a data para 15 dias no futuro
    data_atual = datetime.date.today()

    dados_15_dias = []

    # Fazer a chamada à API para os dados de previsão
    url = "https://api.open-meteo.com/v1/forecast"
    for i in range(15):
...
# Função para buscar dados de 7 dias
def get_7days_futuro(response, output_file):
    # Obter as coordenadas da cidade fornecida pelo user
    geolocator = Nominatim(user_agent="my_geocoder")
    location = geolocator.geocode(response)
    if location is None:
    ...
    # Definir a data atual e a data para 7 dias no futuro
    data_atual = datetime.date.today()
    ...
    for i in range(7):
        ...
        # Extrair os valores relevantes dos dados de resposta
        Temperatura_max = response.Daily().Variables(0).ValuesAsNumpy()
        Temperatura_min = response.Daily().Variables(1).ValuesAsNumpy()
     

        # Iterar sobre os valores de cada hora do dia
        for j in range(len(Temperatura_max)):
            # Organizar os dados da hora no formato desejado
            dado_dia = {
                "Data": data_consulta.isoformat(),
                "Temperatura Max(C)": Temperatura_max[j],
                "Temperatura Min(C)": Temperatura_min[j],
            }
            dados_7_dias.append(dado_dia)
    
    # Converter os dados para DataFrame do Pandas
    df = pd.DataFrame(dados_7_dias)

    # Salvar os dados em um arquivo CSV
    df.to_csv(output_file, index=False)
    

# Função para exibir informacoes atuais
def informacao_atual(response, frame):
    ...
    dia_ou_noite = response.Current().Variables(2).Value()
    chuva_atual = response.Current().Variables(3).Value()
    nuvem_ou_limpo = response.Current().Variables(4).Value()
    vento_atual = response.Current().Variables(5).Value()
    ...
    tree = ttk.Treeview(info_frame, columns=list(current_data.columns), show="headings")
    ...
    # Determinar a imagem a ser exibida com base nas condições
    if dia_ou_noite == 1:
        if nuvem_ou_limpo < 30:
            caminho_imagem = "imagens/ceu_dia_limpo.jpg"
        elif 30 <= nuvem_ou_limpo < 70:
            if chuva_atual > 0:
                caminho_imagem = "imagens/ceu_dia_meia_nuvem_chuva.jpg"
            else:
                caminho_imagem = "imagens/ceu_dia_meia_nuvem.jpg"
        else:
            if chuva_atual > 0:
                caminho_imagem = "imagens/ceu_dia_nublado_chuva.jpg"
            else:
                caminho_imagem = "imagens/ceu_dia_nublado.jpg"
    else:
        if nuvem_ou_limpo < 30:
            caminho_imagem = "imagens/ceu_noite_limpo.jpg"
        elif 30 <= nuvem_ou_limpo < 70:
            if chuva_atual > 0:
                caminho_imagem = "imagens/ceu_noite_meia_nublado_chuva.jpg"
            else:
                caminho_imagem = "imagens/ceu_noite_meia_nublado.jpg"
        else:
            caminho_imagem = "imagens/ceu_noite_nublado.jpg"

    exibir_imagem(frame, caminho_imagem)
...
# Função para exibir informações diárias
def informacao_diaria(file, frame):
    # Carregar os dados do arquivo CSV em um DataFrame
    df = pd.read_csv(file)
    ...
    # Crear um frame para Treeview
    tree_frame = Frame(frame)
    tree_frame.pack(side="top", fill="both", expand=True)

    # Criar e configurar o widget Treeview
    tree = ttk.Treeview(tree_frame, columns=list(df.columns), show="headings")
    for column in df.columns:
        tree.heading(column, text=column)
        tree.column(column, width=100)

    # Colocar data na Treeview
    for _, row in df.iterrows():
        tree.insert("", "end", values=list(row))

    tree.pack(expand=True, fill='both')

    # Crear um frame para o gráfico
    graph_frame = Frame(frame)
    graph_frame.pack(side="bottom", fill="both", expand=True)

    # Gerar o gráfico da temperatura
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.plot(df['Data'], df['Temperatura Max(C)'], label='Max Temperatura (C)', color='red')
    ax.plot(df['Data'], df['Temperatura Min(C)'], label='Min Temperatura (C)', color='blue')
...
def abrir_aba_informacoes(response):
    ...
    def selecionar_aba(event, response):
        ...
        if aba_index == 0:
            ...
            informacao_diaria("Informacao_7dias_futuro.txt", frame_diaria)
        elif aba_index == 3:
...
def iniciar_interface():
    ...
    def buscar_clima():
        ...
        # Converteras horas para inteiros depois de passar nas verificações acima
        ...
        if response:
            ...
            get_7days_futuro(city_name, "Informacao_7dias_futuro.txt")
    ...
    # Função para buscar o clima histórico
    ...
    # Função para poder alternar as buscas
    ...
    root.protocol("WM_DELETE_WINDOW", sair_tudo)
