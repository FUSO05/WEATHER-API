    ...
    for indice, linha in dados.iterrows():
        ...
        if temperatura_anterior is not None:
            ...
            if temperatura_atual > temperatura_anterior:
                if diff > max_diff_aumento and diff > 2:  # Ajuste o valor do limiar conforme necessário
                ...
                if diff > max_diff_queda and diff > 2:  # Ajuste o valor do limiar conforme necessário
                    max_diff_queda = diff
...
def identificar_condicoes_inundacoes(dados):
    ...
    for _, linha in dados.iterrows():
        chuva_atual = linha['Chuva (mm)']
        if chuva_atual > 50:  # Limiar para identificar condições propícias para inundações
