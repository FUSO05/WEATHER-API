...
from tkinter import Tk, Toplevel, Text, Scrollbar, END
...
def create_popup_window(master):
    ...
    text = tk.Text(popup, wrap="word", yscrollcommand=scrollbar.set)
        ...
        if temperatura_anterior is not None:
            ...
            if temperatura_atual > temperatura_anterior:
                if diff > max_diff_aumento and diff > 4:  # Ajuste o valor do limiar conforme necessário
                    ...
                    message = f"Aumento abrupto na temperatura em {linha['Data']} | {linha['Hora']}: {temperatura_anterior} -> {temperatura_atual}"
                ...
                if diff > max_diff_queda and diff > 4:  # Ajuste o valor do limiar conforme necessário
                    ...
                    message = f"Queda abrupta na temperatura em {linha['Data']} | {linha['Hora']}: {temperatura_anterior} -> {temperatura_atual}"
                    add_message(popup_window, message)
        ...
        if humidade_anterior is not None:
            ...
            if humidade_atual > humidade_anterior:
                if diff > max_diff_aumento and diff > 20:  # Ajuste o valor do limiar conforme necessário
                    max_diff_aumento = diff
                    max_diff_date_aumento = linha['Data']
                    message = f"Aumento abrupto na humidade em {linha['Data']} | {linha['Hora']}: {humidade_anterior} -> {humidade_atual}"
                    add_message(popup_window, message)
            elif humidade_atual < humidade_anterior:
                if diff > max_diff_queda and diff > 20:  # Ajuste o valor do limiar conforme necessário
                    max_diff_queda = diff
                    max_diff_date_queda = linha['Data']
                    message = f"Queda abrupta na humidade em {linha['Data']} | {linha['Hora']}: {humidade_anterior} -> {humidade_atual}"
                    add_message(popup_window, message)
        ...
        if chuva_anterior is not None:
            ...
            if chuva_atual > chuva_anterior:
                if diff > max_diff_aumento and diff > 5:  # Ajuste o valor do limiar conforme necessário
                    max_diff_aumento = diff
                    max_diff_date_aumento = linha['Data']
                    message = f"Aumento abrupto na quantidade de chuva em {linha['Data']} | {linha['Hora']}: {chuva_anterior} -> {chuva_atual}"
                    add_message(popup_window, message)
            elif chuva_atual < chuva_anterior:
                if diff > max_diff_queda and diff > 5:  # Ajuste o valor do limiar conforme necessário
                    max_diff_queda = diff
                    max_diff_date_queda = linha['Data']
                    message = f"Queda abrupta na quantidade de chuva em {linha['Data']} | {linha['Hora']}: {chuva_anterior} -> {chuva_atual}"
                    add_message(popup_window, message)
        ...
        if vento_anterior is not None:
            ...
            if vento_atual > vento_anterior:
                if diff > max_diff_aumento and diff > 10:  # Ajuste o valor do limiar conforme necessário
                    max_diff_aumento = diff
                    max_diff_date_aumento = linha['Data']
                    message = f"Aumento abrupto na velocidade do vento em {linha['Data']} | {linha['Hora']}: {vento_anterior} -> {vento_atual}"
                    add_message(popup_window, message)
            elif vento_atual < vento_anterior:
                if diff > max_diff_queda and diff > 10:  # Ajuste o valor do limiar conforme necessário
                    max_diff_queda = diff
                    max_diff_date_queda = linha['Data']
                    message = f"Queda abrupta na velocidade do vento em {linha['Data']} | {linha['Hora']}: {vento_anterior} -> {vento_atual}"
                    add_message(popup_window, message)
...
def buscar_dados_historicos(city_nameh, start_dateh, end_dateh):
    ...
    btn_voltar = tk.Button(window, text="Sair", command=lambda: [window.destroy(), root.deiconify()], width=20)
...
def iniciar_interface():
    global root, cidade_entry, hora_inicial_entry, hora_final_entry, var_busca, frame_clima, frame_clima_historico, popup_window
    root = tk.Tk()
    ...
    def buscar_clima():
        ...
        if response:
            ...
            root.withdraw()
            popup_window = tk.Toplevel(root)  # Criar a janela de pop-up apenas se houver dados para mostrar
    ...
    email_label = tk.Label(frame_clima, text="Insira seu email:")
    email_label.pack(pady=10)
    email_entry = tk.Entry(frame_clima, width=30)
    email_entry.pack(pady=5)

    # Criar um botão para confirmar o email
    confirmar_button = tk.Button(frame_clima, text="Confirmar", command=confirmar_email)
